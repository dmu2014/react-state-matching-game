...
class App extends Component{
  ...
  startGame(numTiles){
    this.setState((state) => {state.playing=true, state.previousTileIndex=null, state.toBeCleared=null, state.tiles = createTiles(state.numTiles, this.handleTileClicked)})      
      //{playing:true, previousTileIndex:null, toBeCleared:null, tiles:createTiles(numTiles)});
  }

  handleNumTileChange = (num) => {
    this.setState((state) => {state.numTiles= num, state.playing=false, state.tiles=[]});
  }
